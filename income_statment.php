<html>
	...
	<body>
        <?php require("core/navbar.php");
			...
			if(!empty($_POST["rounds"])){
				$round = $_POST["rounds"];
				if($round >= 0){
					$_SESSION['accounts']["21.10"] = $round;
				}else{
					$_SESSION['accounts']["39.10"] = $round;
				}
			}			
		
		?>
			<div class="balance_sheet_utils">
    ...
    <script>
		...
		$(document).ready(function(){
			$.each($(".income_statement_row"), function(key,value){
				//Cerco le righe che non hanno row_parents
				if($($(value).children()[1]).html() === ""){
					find = true
					$.each($('[row_id_parents =' + $(value).attr("row_id") +']'),function(k,v){
						if($($(v).children()[1]).html() != "-" && $($(v).children()[1]).html() != "") find = false
					})
					
					if (find){
						$(value).addClass("row_hidden")
						$.each($('[row_id_parents =' + $(value).attr("row_id") +']'),function(k,v){
							$(v).addClass("row_hidden")
						})
					}else{
						$.each($(".income_statement_row"),function(k,v){
							if(($($(v).children()[1]).html() === "-" ) || ($($(v).children()[1]).html() === "<b>-</b>" )) $(v).addClass("row_hidden")
						})
					}
				}
			})
